...
void Ex12opengl::initializeGL()
{
   ...
   //addShader("",":/gaussianblur.frag");
   addShader("",":/invertcolors.frag");
   addShader("",":/vigrette.frag");
   addShader("",":/mix.frag");
   addShader("",":/displacement.frag");
   addShader("",":/oldtv.frag");
   //addShader("",":/swirl.frag");
...
void Ex12opengl::paintGL()
{
   ...
   if (mode)
   {
      ...
      shader[mode]->bind();
      //rsolution
      QVector2D res=QVector2D(width(),height());

      //  Set shader increments
      shader[mode]->setUniformValue("dX",(float)1.0/Cw);
      shader[mode]->setUniformValue("dY",(float)1.0/Ch);
      shader[mode]->setUniformValue("time",getTime());
      shader[mode]->setUniformValue("resolution",res);
      //shader[mode]->setUniformValue("ProjectionMatrix",view);
